folder('dotnet-rel2.0') {}

def generatorJob = job('dotnet-rel2.0/generator') {
  logRotator {
    daysToKeep(7)
  }
  triggers {
...
''')
  ...
  publishers {
    downstreamParameterized {
      trigger('dotnet-rel2.0/coreclr, dotnet-rel2.0/corefx, dotnet-rel2.0/core-setup') {
        condition('SUCCESS')
        parameters {
          propertiesFile('env.dotnet_build_id')
...
['coreclr', 'corefx', 'core-setup'].each { name ->
  ...
  def newJob = job("dotnet-rel2.0/${name}") {
    scm {
      git {
        remote {
          github("dotnet/${name}")
  ...
  newJob.with {
    publishers {
      downstream("dotnet-rel2.0/upload-${name}")
    }
  }
}
...
['coreclr', 'corefx', 'core-setup'].each { name ->
  def uploadJob = job("dotnet-rel2.0/upload-${name}") {
    wrappers {
      timestamps()
      // Add a pre-build wipe-out
      preBuildCleanup()
